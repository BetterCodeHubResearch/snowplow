    ...
    case (Some(body),_)                     => {
      if (body.isEmpty) {
        ...
        try {
          ...
          lookupSchema(Some("status_change"), VendorName, EventSchemaMap) match {
            ...
            case Success(schema) => {
              NonEmptyList(RawEvent(
                api          = payload.api,
                parameters   = toUnstructEventParams(TrackerVersion, qsParams, schema, camelize(event), "srv"),
          ...
          case NonFatal(e) => s"${VendorName} incorrect event string : [${JU.stripInstanceEtc(e.getMessage).orNull}]".failNel
        }
      }
    }
  }
