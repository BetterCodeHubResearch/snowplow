...
def toRawEvents(payload: CollectorPayload)(implicit resolver: Resolver): ValidatedRawEvents = {
      ...
      case (Some(body), Some(ct))                                   => {
        ...
        try {
          ...
          bodyMap.get("event") match {
            ...
            case Some(eventType) => (payloadBodyToEvent(bodyMap)) match {
  ...
  */
  private def mutateMailgunEvent(json: JValue): JValue = {
    ...
    (jsonCamelCase \ "attachmentCount").extractOpt[String].map(ac =>  ("attachmentCount" -> JInt(ac.toInt)) ~ dropFields).getOrElse(dropFields)
  }
      ...
      case (Some(timestamp), Some(token), Some(signature)) => {
        try {
          ...
          case e: JsonParseException =>
            s"${VendorName} event string failed to parse into JSON: [${JU.stripInstanceEtc(e.toString).orNull}]".fail
          case NonFatal(e) =>
            s"${VendorName} incorrect event string: [${JU.stripInstanceEtc(e.getMessage).orNull}]".fail
        }
      }
    }
  }
